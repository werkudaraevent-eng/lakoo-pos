...
const ProductForm = ({ product, onSave, onCancel, isNew }) => {
  const initSizes = product
    ? Object.entries(product.sizes).map(([label, qty]) => ({ label, qty }))
  ...
  const [sizes, setSizes] = React.useState(initSizes);
  const [newSize, setNewSize] = React.useState('');
  const [saved, setSaved] = React.useState(false);
  const [deleteConfirm, setDeleteConfirm] = React.useState(false);

  const cats = ['Atasan', 'Bawahan', 'Outerwear', 'Aksesoris'];
  const totalStock = sizes.reduce((s, z) => s + (parseInt(z.qty) || 0), 0);

  const addSize = () => {
    const label = newSize.trim().toUpperCase();
    if (!label || sizes.find(s => s.label === label)) return;
    setSizes(prev => [...prev, { label, qty: 0 }]);
    setNewSize('');
  };

  const removeSize = label => setSizes(prev => prev.filter(s => s.label !== label));
  const setQty = (label, val) => setSizes(prev => prev.map(s => s.label === label ? { ...s, qty: Math.max(0, parseInt(val) || 0) } : s));

  const handleSave = () => {
    if (!form.name || !form.sku || !form.price) return;
    const sizesObj = {};
    sizes.forEach(s => { sizesObj[s.label] = parseInt(s.qty) || 0; });
    onSave({ ...form, price: parseInt(String(form.price).replace(/\D/g, '')) || 0, sizes: sizesObj });
    setSaved(true);
  ...
  return (
    <div className="content" style={{ maxWidth: 860 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24, flexWrap: 'wrap' }}>
        <button className="btn btn-ghost btn-sm" onClick={onCancel}>← Kembali</button>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 18, fontWeight: 800 }}>{isNew ? 'Tambah Produk Baru' : `Edit: ${product.name}`}</div>
          {!isNew && <div style={{ fontSize: 12.5, color: 'var(--text-2)', marginTop: 2 }}>SKU {product.sku}</div>}
        </div>
        {!isNew && (
          <button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }} onClick={() => setDeleteConfirm(true)}>
            <Icon name="trash" size={13} /> Hapus Produk
          </button>
        )}
        <button className="btn btn-primary" style={{ minWidth: 140, height: 40 }} onClick={handleSave}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Info Dasar */}
          <div className="card">
            <div style={{ fontSize: 13.5, fontWeight: 800, marginBottom: 16, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.5px', fontSize: 11 }}>INFORMASI PRODUK</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Nama Produk *</label>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>SKU *</label>
                  <input className="input" placeholder="TSH-001" value={form.sku} onChange={e => setForm({ ...form, sku: e.target.value.toUpperCase() })} style={{ fontFamily: 'monospace', fontSize: 13 }} />
                </div>
                <div>
                  <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Kategori</label>
              ...
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Harga Jual (Rp) *</label>
                <div style={{ position: 'relative' }}>
                  <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', fontSize: 13.5, fontWeight: 600, color: 'var(--text-2)' }}>Rp</span>
                  <input className="input" placeholder="85.000" value={form.price ? Number(form.price).toLocaleString('id-ID') : ''} onChange={e => setForm({ ...form, price: e.target.value.replace(/\D/g, '') })} style={{ paddingLeft: 36, fontSize: 14, fontWeight: 700 }} />
                </div>
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-2)', display: 'block', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Deskripsi</label>
          ...
          <div className="card">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>UKURAN & STOK</div>
              <span style={{ fontSize: 12.5, color: 'var(--text-2)' }}>Total: <strong style={{ color: 'var(--text)' }}>{totalStock} unit</strong></span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 14 }}>
              {sizes.map(sz => (
                <div key={sz.label} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', background: 'var(--surface)', borderRadius: 8 }}>
                  <div style={{ width: 44, height: 32, borderRadius: 6, background: '#fff', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, flexShrink: 0 }}>{sz.label}</div>
                  <div style={{ flex: 1, fontSize: 13, color: 'var(--text-2)', fontWeight: 500 }}>Stok tersedia</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div className="qty-btn" style={{ width: 28, height: 28 }} onClick={() => setQty(sz.label, sz.qty - 1)}>−</div>
                    <input
                      type="number" min={0}
                      value={sz.qty}
                      onChange={e => setQty(sz.label, e.target.value)}
                      style={{ width: 60, textAlign: 'center', padding: '4px 6px', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'var(--font)', fontWeight: 700, fontSize: 14 }}
                    />
                    <div className="qty-btn" style={{ width: 28, height: 28 }} onClick={() => setQty(sz.label, sz.qty + 1)}>+</div>
                  </div>
                  ...
                  <button onClick={() => removeSize(sz.label)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-3)', padding: 4, display: 'flex', alignItems: 'center' }}>
                    <Icon name="x" size={14} />
                  </button>
                </div>
              ))}
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <button className="btn btn-secondary btn-sm" style={{ flexShrink: 0 }} onClick={addSize}><Icon name="plus" size={13} /> Tambah</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ aspectRatio: '1', background: 'var(--surface)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 10, cursor: 'pointer' }}>
              <svg width="48" height="48" fill="none" stroke="var(--text-3)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
                <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
              </svg>
              <div style={{ fontSize: 13, color: 'var(--text-2)', fontWeight: 600 }}>Upload Foto Produk</div>
              <div style={{ fontSize: 11.5, color: 'var(--text-3)' }}>JPG, PNG — maks 5 MB</div>
            </div>
          ...
          {/* Status card */}
          <div className="card">
            <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 14 }}>STATUS PRODUK</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {[
                { label: 'Tampilkan di Katalog', desc: 'Produk bisa dilihat & dijual', key: 'visible', def: true },
                { label: 'Tersedia di Bazar', desc: 'Bisa dialokasikan ke event', key: 'bazarReady', def: true },
              ].map(opt => {
                const [on, setOn] = React.useState(opt.def);
                return (
                  <div key={opt.key} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 700 }}>{opt.label}</div>
                      <div style={{ fontSize: 11.5, color: 'var(--text-2)' }}>{opt.desc}</div>
                    </div>
                    <div onClick={() => setOn(!on)} style={{ width: 40, height: 22, borderRadius: 11, background: on ? 'var(--accent)' : 'var(--surface-2)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
                      <div style={{ position: 'absolute', top: 2, left: on ? 20 : 2, width: 18, height: 18, borderRadius: '50%', background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.2)', transition: 'left 0.2s' }}></div>
                    </div>
                  </div>
                );
              })}
          ...
          {/* Summary */}
          <div className="card" style={{ background: 'var(--accent-light)', border: '1px solid var(--accent-bg)' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                { label: 'Harga', value: form.price ? 'Rp ' + Number(form.price).toLocaleString('id-ID') : '—' },
                { label: 'Total Stok', value: totalStock + ' unit' },
                { label: 'Ukuran', value: sizes.length > 0 ? sizes.map(s => s.label).join(', ') : '—' },
      ...
      {deleteConfirm && (
        <div className="modal-overlay" onClick={() => setDeleteConfirm(false)}>
          <div className="modal" style={{ width: 380, textAlign: 'center' }} onClick={e => e.stopPropagation()}>
            <div style={{ width: 52, height: 52, borderRadius: '50%', background: 'var(--red-bg)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
              <Icon name="trash" size={22} color="var(--red)" />
            </div>
            <div style={{ fontSize: 17, fontWeight: 800, marginBottom: 8 }}>Hapus Produk?</div>
            <div style={{ fontSize: 13.5, color: 'var(--text-2)', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button className="btn" style={{ flex: 1, background: 'var(--red)', color: '#fff' }} onClick={() => { onCancel(); }}>Hapus</button>
            </div>
          </div>
        </div>
      )}
...
const Katalog = () => {
  const [search, setSearch] = React.useState('');
  const [cat, setCat] = React.useState('Semua');
  const [view, setView] = React.useState('list'); // 'list' | 'edit' | 'new'
  ...
  const handleSave = updated => {
    if (view === 'new') {
      setProducts(prev => [...prev, { ...updated, id: Date.now(), img: updated.category.toLowerCase() }]);
    } else {
  ...
  return (
    <div className="content">
      <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <button className="btn btn-primary" onClick={() => setView('new')}><Icon name="plus" size={14} /> Tambah Produk</button>
      </div>
      <div className="cat-filter">
        {cats.map(c => <div key={c} className={`cat-chip${cat === c ? ' active' : ''}`} onClick={() => setCat(c)}>{c}</div>)}
      </div>
          ...
          return (
            <div key={p.id} className="card card-sm">
              ...
              <div style={{ marginTop: 10, display: 'flex', gap: 6 }}>
                <button className="btn btn-secondary btn-sm" style={{ flex: 1 }} onClick={() => { setEditProduct(p); setView('edit'); }}>
                  <Icon name="edit" size={12} /> Edit
                </button>
                <button className="btn btn-ghost btn-sm btn-icon" style={{ color: 'var(--red)' }} onClick={() => setProducts(prev => prev.filter(x => x.id !== p.id))}>
...
// ── Tambah Stok Modal ──────────────────────────────────
const TambahStokModal = ({ product, onClose, onSave }) => {
  const [deltas, setDeltas] = React.useState(() => {
    const d = {};
    Object.keys(product.sizes).forEach(s => { d[s] = 0; });
    return d;
  });
  const total = Object.values(deltas).reduce((a, b) => a + b, 0);
  const set = (s, v) => setDeltas(prev => ({ ...prev, [s]: Math.max(0, parseInt(v) || 0) }));
      ...
      <div className="modal" style={{ width: 420 }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 4 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 20 }}>{product.name} · <span className="tag">{product.sku}</span></div>
          </div>
          <button className="btn btn-ghost btn-icon" onClick={onClose}><Icon name="x" size={15} /></button>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
          {Object.entries(product.sizes).map(([s, current]) => (
            <div key={s} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: 'var(--surface)', borderRadius: 8 }}>
              <div style={{ width: 40, height: 30, borderRadius: 6, background: '#fff', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 12, flexShrink: 0 }}>{s}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 600 }}>Ukuran {s}</div>
                <div style={{ fontSize: 11.5, color: 'var(--text-2)' }}>Stok saat ini: <strong>{current}</strong></div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 12, color: 'var(--text-2)', fontWeight: 600 }}>+ Tambah</span>
                <div className="qty-btn" style={{ width: 26, height: 26 }} onClick={() => set(s, deltas[s] - 1)}>−</div>
                <input type="number" min={0} value={deltas[s]} onChange={e => set(s, e.target.value)}
                  style={{ width: 52, textAlign: 'center', padding: '4px', border: '1px solid var(--border)', borderRadius: 6, fontFamily: 'var(--font)', fontWeight: 700, fontSize: 14 }} />
                <div className="qty-btn" style={{ width: 26, height: 26 }} onClick={() => set(s, deltas[s] + 1)}>+</div>
              </div>
              <div style={{ minWidth: 60, textAlign: 'right', fontSize: 13, fontWeight: 700, color: 'var(--green)' }}>
                {deltas[s] > 0 ? `→ ${current + deltas[s]}` : ''}
              </div>
            </div>
          ))}
        </div>
        <div style={{ background: 'var(--accent-light)', border: '1px solid var(--accent-bg)', borderRadius: 8, padding: '10px 14px', marginBottom: 16, display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
          <span style={{ color: 'var(--text-2)', fontWeight: 500 }}>Total penambahan</span>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary" style={{ flex: 2, height: 42 }} disabled={total === 0} onClick={() => { onSave(product.id, deltas); onClose(); }}>
...
const Stok = () => {
  ...
  const [products, setProducts] = React.useState(PRODUCTS);
  const [tambahModal, setTambahModal] = React.useState(null);
  const [editProduct, setEditProduct] = React.useState(null);
  ...
  const totalStock = p => Object.values(p.sizes).reduce((a, b) => a + b, 0);
  const getStatus = total => total === 0 ? 'Habis' : total <= 3 ? 'Hampir Habis' : total <= 10 ? 'Terbatas' : 'Tersedia';
  const getBadge = total => total === 0 ? 'badge-red' : total <= 3 ? 'badge-red' : total <= 10 ? 'badge-amber' : 'badge-green';

  const filtered = products.filter(p =>
    p.name.toLowerCase().includes(search.toLowerCase()) &&
    (catFilter === 'Semua' || p.category === catFilter) &&
    (statusFilter === 'Semua' || getStatus(totalStock(p)) === statusFilter)
  );

  const handleTambahSave = (productId, deltas) => {
    setProducts(prev => prev.map(p => {
      if (p.id !== productId) return p;
      const newSizes = { ...p.sizes };
      Object.entries(deltas).forEach(([s, d]) => { newSizes[s] = (newSizes[s] || 0) + d; });
    ...
    <div className="content">
      <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <button className={`btn ${showFilter ? 'btn-primary' : 'btn-secondary'}`} onClick={() => setShowFilter(!showFilter)}>
          <Icon name="filter" size={14} /> Filter {(catFilter !== 'Semua' || statusFilter !== 'Semua') ? '●' : ''}
        </button>
        <button className="btn btn-primary" onClick={() => setTambahModal('picker')}>
          <Icon name="plus" size={14} /> Tambah Stok
        </button>
      </div>

      {/* Filter bar */}
      {showFilter && (
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', marginBottom: 14, display: 'flex', gap: 20, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Kategori</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
              {cats.map(c => <div key={c} className={`cat-chip${catFilter === c ? ' active' : ''}`} style={{ fontSize: 12, padding: '4px 10px' }} onClick={() => setCatFilter(c)}>{c}</div>)}
            </div>
          </div>
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-2)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Status Stok</div>
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
        {[
          { label: 'Total Produk', val: products.length },
          { label: 'Hampir Habis', val: products.filter(p => { const t = totalStock(p); return t > 0 && t <= 3; }).length, warn: true },
          { label: 'Stok Habis', val: products.filter(p => totalStock(p) === 0).length, danger: true },
          { label: 'Total Unit', val: products.reduce((s, p) => s + totalStock(p), 0) },
        ].map((k, i) => (
          <div key={i} className="card card-sm" style={{ flex: 1, padding: '10px 14px' }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: k.danger ? 'var(--red)' : k.warn ? 'var(--accent)' : 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '0.4px', marginBottom: 4 }}>{k.label}</div>
            <div style={{ fontSize: 20, fontWeight: 800, color: k.danger && k.val > 0 ? 'var(--red)' : k.warn && k.val > 0 ? 'var(--accent)' : 'var(--text)' }}>{k.val}</div>
          </div>
        ))}
      </div>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>Produk</th><th>SKU</th><th>Kategori</th><th>Ukuran & Stok</th><th>Total</th><th>Status</th><th>Aksi</th>
            </tr></thead>
            <tbody>
              {filtered.map(p => {
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button className="btn btn-secondary btn-sm" onClick={() => { setEditProduct(p); setView('edit'); }}>
                          <Icon name="edit" size={12} /> Edit
                        </button>
      ...
      {tambahModal && tambahModal !== 'picker' && (
        <TambahStokModal product={tambahModal} onClose={() => setTambahModal(null)} onSave={handleTambahSave} />
      )}

      {/* Picker: choose product to add stock */}
      {tambahModal === 'picker' && (
        <div className="modal-overlay" onClick={() => setTambahModal(null)}>
          <div className="modal" style={{ width: 460 }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              ...
              <button className="btn btn-ghost btn-icon" onClick={() => setTambahModal(null)}><Icon name="x" size={15} /></button>
            </div>
            <div style={{ fontSize: 13, color: 'var(--text-2)', marginBottom: 14 }}>Pilih produk yang ingin ditambah stoknya:</div>
            <div style={{ maxHeight: 360, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {products.map(p => {
                const tot = totalStock(p);
                return (
                  <div key={p.id} onClick={() => setTambahModal(p)} style={{
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-2)' }}>{p.sku} · {p.category}</div>
                    </div>
    ...
    <div className="content">
      ...
      <div style={{display:'flex',gap:12,marginBottom:16}}>
        {[
          ...
          {label:'Total Pendapatan', val: fmtRp(totalRev)},
          {label:'Total Item', val: filtered.reduce((s,t)=>s+t.items,0)},
        ].map((k,i)=>(
          <div key={i} className="card card-sm" style={{flex:1}}>
            <div className="kpi-label">{k.label}</div>
      ...
      <div className="card" style={{padding:0,overflow:'hidden'}}>
        <div className="table-wrap">
          <table>
            <thead><tr>
              <th>ID Transaksi</th><th>Tanggal & Waktu</th><th>Produk</th><th>Items</th><th>Total</th><th>Metode</th><th>Status</th><th></th>
            </tr></thead>
            <tbody>
              {filtered.map(t=>(
                <tr key={t.id} style={{cursor:'pointer'}} onClick={()=>setDetail(t)}>
                  ...
                  <td className="text-sm text-muted" style={{maxWidth:200}}>{t.products.slice(0,2).join(', ')}{t.products.length>2?` +${t.products.length-2}`:''}</td>
                  <td><span style={{fontWeight:600}}>{t.items}</span></td>
                  <td><span style={{fontWeight:800}}>{fmtRp(t.total)}</span></td>
                  <td><span className="badge badge-gray">{t.method}</span></td>
                  <td><span className="badge badge-green">{t.status}</span></td>
                  <td><button className="btn btn-ghost btn-sm" style={{color:'var(--text-2)'}}>Detail →</button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {detail && (
        <div className="modal-overlay" onClick={()=>setDetail(null)}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:16,width:380,maxWidth:'90vw',boxShadow:'0 20px 60px rgba(0,0,0,0.15)',overflow:'hidden'}}>
            {/* Receipt header */}
            <div style={{background:'var(--text)',padding:'24px 24px 20px',textAlign:'center'}}>
              <div style={{fontSize:22,fontWeight:800,color:'#fff',letterSpacing:'-0.5px'}}>nyam.</div>
              <div style={{fontSize:11,color:'rgba(255,255,255,0.5)',marginTop:3,letterSpacing:'0.5px',textTransform:'uppercase'}}>Point of Sale</div>
            </div>
            {/* Receipt body */}
            <div style={{padding:'20px 24px'}}>
              <div style={{textAlign:'center',marginBottom:16}}>
                <div style={{fontSize:12,color:'var(--text-2)',marginBottom:4}}>{detail.date}</div>
                <div style={{fontSize:20,fontWeight:800,color:'var(--accent)',letterSpacing:'-0.5px'}}>{detail.id}</div>
              </div>
              <div style={{borderTop:'1px dashed var(--border)',borderBottom:'1px dashed var(--border)',padding:'14px 0',marginBottom:14}}>
                {detail.products.map((p,i)=>{
                  const prod = PRODUCTS.find(x=>p.includes(x.name.split(' ').slice(0,3).join(' ')));
                  const price = prod?.price || Math.round(detail.total/detail.items);
                  return (
                    <div key={i} style={{display:'flex',justifyContent:'space-between',marginBottom:8,fontSize:13.5}}>
                      <span style={{flex:1,paddingRight:12}}>{p}</span>
                      <span style={{fontWeight:700,flexShrink:0}}>{fmtRp(price)}</span>
                    </div>
                  );
                })}
              </div>
              <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:13.5}}>
                  ...
                  <span style={{fontWeight:600}}>{fmtRp(Math.round(detail.total/1.01))}</span>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:13.5}}>
                  <span style={{color:'var(--text-2)'}}>Pajak (1%)</span>
                  <span style={{fontWeight:600}}>{fmtRp(detail.total - Math.round(detail.total/1.01))}</span>
                </div>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:16,fontWeight:800,paddingTop:8,borderTop:'1px solid var(--border)'}}>
                  <span>Total</span>
                  <span style={{color:'var(--accent)'}}>{fmtRp(detail.total)}</span>
                </div>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',padding:'10px 14px',background:'var(--surface)',borderRadius:8,marginBottom:16,fontSize:13}}>
                <span style={{color:'var(--text-2)',fontWeight:500}}>Metode Bayar</span>
                <span className={`badge ${detail.method==='QRIS'?'badge-blue':detail.method==='Transfer'?'badge-green':'badge-gray'}`}>{detail.method}</span>
              </div>
              <div style={{textAlign:'center',fontSize:12,color:'var(--text-3)',marginBottom:16}}>
                Terima kasih telah berbelanja di Nyam!<br/>IG: @nyamstore · nyamstore.id
              </div>
              <div style={{display:'flex',gap:8}}>
                <button className="btn btn-secondary" style={{flex:1}} onClick={()=>setDetail(null)}>Tutup</button>
                <button className="btn btn-primary" style={{flex:1}}><Icon name="print" size={13}/> Cetak Struk</button>
              </div>
            </div>
          </div>
        </div>
