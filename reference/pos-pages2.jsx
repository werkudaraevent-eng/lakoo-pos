  ...
  if (view === 'detail' && selected) {
    ...
    return (
      <div className="content">
        ...
        {stockTab==='list' && (
          <div className="card" style={{padding:0,overflow:'hidden'}}>
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:13.5}}>
              <thead><tr>
                {['Produk','SKU','Stok Toko','Terjual di Event','Sisa Event'].map(h=>(
                  <th key={h} style={{textAlign:'left',padding:'10px 14px',fontSize:11.5,fontWeight:700,color:'var(--text-2)',borderBottom:'1px solid var(--border)',textTransform:'uppercase',letterSpacing:'0.5px'}}>{h}</th>
                ))}
              </tr></thead>
              <tbody>
                {PRODUCTS.slice(0,8).map(p=>{
                  ...
                  const sold = Math.floor(alloc * 0.4); // simulasi terjual
                  const sisa = ev.stockMode==='custom' ? alloc - sold : tokoStock;
                  return (
                    <tr key={p.id} style={{borderBottom:'1px solid var(--border)'}}>
                      <td style={{padding:'12px 14px',fontWeight:600}}>{p.name}</td>
                      <td style={{padding:'12px 14px'}}><span className="tag">{p.sku}</span></td>
                      <td style={{padding:'12px 14px',color:'var(--text-2)'}}>{tokoStock}</td>
                      ...
                      <td style={{padding:'12px 14px'}}>
                        <span style={{fontWeight:600,color:sisa<=3?'var(--red)':'var(--text)'}}>{sisa}</span>
                        {sisa<=3&&<span style={{marginLeft:6,fontSize:11,color:'var(--red)',fontWeight:600}}>Hampir habis</span>}
                      </td>
                    </tr>
                  );
                })}
        ...
        {/* Tab: Alokasi Stok — set berapa unit dibawa ke event */}
        {stockTab==='allocate' && (
          <>
            {ev.stockMode==='inherit' ? (
              ...
              <>
                <div style={{marginBottom:12,padding:'10px 14px',background:'var(--accent-light)',borderRadius:10,border:'1px solid var(--accent-bg)',fontSize:13,color:'var(--accent)'}}>
                ...
                <div className="card" style={{padding:0,overflow:'hidden'}}>
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:13.5}}>
                    <thead><tr>
                      {['Produk','SKU','Stok Toko','Alokasi ke Event','Sisa di Toko'].map(h=>(
                        <th key={h} style={{textAlign:'left',padding:'10px 14px',fontSize:11.5,fontWeight:700,color:'var(--text-2)',borderBottom:'1px solid var(--border)',textTransform:'uppercase',letterSpacing:'0.5px'}}>{h}</th>
                      ))}
                    </tr></thead>
                    <tbody>
                      {PRODUCTS.slice(0,8).map(p=>{
                        ...
                        return (
                          <tr key={p.id} style={{borderBottom:'1px solid var(--border)'}}>
                            ...
                            <td style={{padding:'12px 14px',color:'var(--text-2)'}}>{tokoStock}</td>
                            <td style={{padding:'12px 14px'}}>
                              <input type="number" style={{width:80,padding:'5px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:13,fontFamily:'var(--font)',background:'#fff'}} defaultValue={alloc} min={0} max={tokoStock} />
                            </td>
                            <td style={{padding:'12px 14px'}}>
                              <span style={{fontWeight:600,color:sisa<=3?'var(--red)':'var(--text)'}}>{sisa}</span>
                              {sisa<=3&&<span style={{marginLeft:6,fontSize:11,color:'var(--red)',fontWeight:600}}>⚠ Tipis</span>}
                            </td>
                          </tr>
                        );
                      })}
